...
export default function Header() {
  return (
    <header className="bg-white shadow-md px-8 py-4 flex justify-between items-center">
      <div className="text-2xl font-bold text-red-500">Sedap</div>
      <nav className="space-x-4">
        <a href="#about" className="hover:text-red-500">Tentang</a>
        <a href="#produk" className="hover:text-red-500">Produk</a>
        <a href="#testimoni" className="hover:text-red-500">Testimoni</a>
        <Link to="/login" className="bg-red-500 text-white px-4 py-1 rounded">Login</Link>
        <Link to="/register" className="text-red-500 border border-red-500 px-4 py-1 rounded">Register</Link>
      </nav>
    </header>
  );
}